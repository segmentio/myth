...
function plugin(options){
  return function(stylesheet, rework){
    ...
    var imports = 'undefined' == typeof window ? importer() : noop;
    ...
    var reworked = rework.toString(options);
    var autoprefixed = prefixes.process(reworked, {
      // not sure why we need this options, but it's required
      from: source,
      to: source
    }).css;

    delete options.source // sourcemap already good at this point.
    rework.obj = Rework(autoprefixed, options).obj;
  };
}
